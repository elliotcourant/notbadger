...
func DefaultOptions(path string) Options {
	// TODO (elliotcourant) This was originally ZSTD and if Cgo is not enabled it would be Snappy. I've removed this for
...
func (opt Options) WithLogger(val timber.Logger) Options {
	opt.Logger = val
	return opt
}
...
// WithNumMemoryTables returns a new Options value with NumMemoryTables set to the given value.
//
// NumMemoryTables sets the maximum number of tables to keep in memory before stalling.
//
// The default value of NumMemoryTables is 5.
func (opt Options) WithNumMemoryTables(val int) Options {
	opt.NumMemoryTables = val
	return opt
}
